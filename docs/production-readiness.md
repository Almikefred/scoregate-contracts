# Production Readiness Review — ScoreGate Score Contract
...
| R4 | Locked release WASM build succeeds | Author | Build failure | `cargo build --target wasm32-unknown-unknown --release -p scoregate-score --locked` log |
