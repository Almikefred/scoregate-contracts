...
//! cargo run -p schema-gen -- --wasm target/wasm32-unknown-unknown/release/scoregate_score.wasm
...
fn main() -> Result<()> {
    ...
    let (source, struct_) = match &wasm_path {
        ...
        None => {
            ...
            (
                "contracts/scoregate-score/src/types.rs (`RiskScore::spec_xdr()`)".to_string(),
                struct_,
            )
        }
    };
