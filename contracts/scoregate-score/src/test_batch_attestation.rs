...
use crate::{
    BatchAttestation, Error, ScoreGateScoreContract, ScoreGateScoreContractClient,
...
/// [`ScoreGateScoreContract::compute_merkle_leaf`].
...
fn test_valid_merkle_batch_accepted() {
    ...
    let leaf0 = env.as_contract(&client.address, || {
        let leaf =
            ScoreGateScoreContract::compute_merkle_leaf(&env, submissions_vec.get(0).unwrap())
                .unwrap();
...
fn test_verify_merkle_proof_standalone() {
    ...
    for index in 0..8u32 {
        ...
        env.as_contract(&contract_id, || {
            ...
            let result = ScoreGateScoreContract::verify_merkle_proof(
                &env,
                &leaf_bn,
                &proof,
                flags,
