...
use crate::{
    Error, ScoreGateScoreContract, ScoreGateScoreContractClient, MaybeScoreAttestation,
...
fn test_deployment_is_domain_separated() {
    ...
    let env = Env::default();
    env.mock_all_auths();
    let deploy_a = env.register_contract(None, ScoreGateScoreContract);
    let deploy_b = env.register_contract(None, ScoreGateScoreContract);
