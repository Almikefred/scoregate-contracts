...
//!
//! Tests register two `ScoreGateScoreContract` instances in the same Soroban
...
use crate::{
    constants::FAILOVER_STALENESS_WINDOW, ScoreGateScoreContract, ScoreGateScoreContractClient,
};
...
fn setup_two<'a>(
) -> (Env, ScoreGateScoreContractClient<'a>, ScoreGateScoreContractClient<'a>, Address, Address) {
    let env = Env::default();
    env.mock_all_auths();
    env.ledger().with_mut(|l| l.timestamp = START_TS);

    let primary_id = env.register_contract(None, ScoreGateScoreContract);
    let secondary_id = env.register_contract(None, ScoreGateScoreContract);

    let primary = ScoreGateScoreContractClient::new(&env, &primary_id);
    let secondary = ScoreGateScoreContractClient::new(&env, &secondary_id);
