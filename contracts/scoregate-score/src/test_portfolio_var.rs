...
struct Setup<'a> {
    env: Env,
    client: ScoreGateScoreContractClient<'a>,
}

fn setup() -> Setup<'static> {
    let env = Env::default();
    env.mock_all_auths();
    let contract = env.register_contract(None, ScoreGateScoreContract);
    let client = ScoreGateScoreContractClient::new(&env, &contract);
    let admin = Address::generate(&env);
    let service = Address::generate(&env);
    client.initialize(&admin, &service);
    let client: ScoreGateScoreContractClient<'static> =
        unsafe { core::mem::transmute(client) };
    Setup { env, client }
}

fn submit(env: &Env, client: &ScoreGateScoreContractClient, wallet: &Address, pair: Symbol, score: u32) {
    client
