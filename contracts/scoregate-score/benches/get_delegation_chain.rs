...
//!
//! Run: `cargo bench -p scoregate-score --bench get_delegation_chain`
//!
...
fn setup_client(env: &Env) -> ScoreGateScoreContractClient<'_> {
    env.mock_all_auths();
    env.budget().reset_unlimited();
    env.ledger().with_mut(|l| l.timestamp = START_TS);

    let contract_id = env.register_contract(None, scoregate_score::ScoreGateScoreContract);
